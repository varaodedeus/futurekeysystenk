...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!username || !password) return res.status(400).json({ success: false, error: 'Campos obrigatórios faltando' });
    ...
    const token = jwt.sign({ userId: user._id.toString(), username: user.username, email: user.email }, JWT_SECRET, { expiresIn: '30d' });
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (!username || !email || !password) return res.status(400).json({ success: false, error: 'Todos os campos são obrigatórios' });
    ...
    if (existing) return res.status(400).json({ success: false, error: 'Username ou email já existe' });

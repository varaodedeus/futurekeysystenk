...
router.post('/create-project', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const db = client.db();
    const projectDoc = { name, description: description || '', userId: new ObjectId(req.user.userId), createdAt: new Date(), stats: { totalKeys: 0, activeKeys: 0, totalValidations: 0 } };
    const result = await db.collection('projects').insertOne(projectDoc);
    res.status(201).json({ success: true, projectId: result.insertedId, project: projectDoc });
  } catch (error) {
...
router.get('/list-projects', authMiddleware, async (req, res) => {
  ...
  try {
    client = await MongoClient.connect(uri);
    const db = client.db();
    const projects = await db.collection('projects').find({ userId: new ObjectId(req.user.userId) }).sort({ createdAt: -1 }).toArray();
...
router.get('/analytics/:projectId', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      analytics[date.toISOString().split('T')[0]] = { keysCreated: 0, keysActivated: 0, executions: 0 };
    }
